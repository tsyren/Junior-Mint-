...
class ActiveSearch::TermIndexer < ActiveSearch::Indexer
  ...
  def perform_setup
    ...
    unless klass.instance_methods.include?(Inflector.pluralize(term_klass).underscore)
      klass.send(:has_and_belongs_to_many, Inflector.pluralize(term_klass).underscore.to_sym)
    end
    ...
    @used_columns << Inflector.pluralize(@term_klass).underscore
  end
