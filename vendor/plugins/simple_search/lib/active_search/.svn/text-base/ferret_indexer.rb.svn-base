...
#    class_tree_restriction = Ferret::QueryParser.new(:fields => [:ar_class]).parse(query_for_class_tree)

    #class_tree_restriction = Ferret::QueryParser.new.parse(query_for_class_tree)
    #sti_filter = Ferret::Search::QueryFilter.new(class_tree_restriction)
    #puts sti_filter.inspect
    # Filtering allows us to have not only different classes in the same STI chain
    # but also different models writing to the same index
    
    query = "contents:(#{query}) AND #{query_for_class_tree}"
    ...
    @ferret_index.search_each(query) do | doc_offset, score|
       ...
       kls = @ferret_index[doc_offset][:ar_class]
       record_class = kls.constantize
       
       # wrap in result
       unless seen.include?([kls, id_to_lookup])
         puts "Found #{record_class} #{id_to_lookup}"
         seen << [kls, id_to_lookup]
         results << ActiveSearch::FerretResult.new(record_class.find(id_to_lookup), lazy, lazy.fields)
       end
    end
  ...
  private
    ...
    def query_for_class_tree
      descendants_or_self = ([klass] + subclasses_of(klass)).collect{|k| k.to_s.downcase }
      "ar_class:(%s)" % descendants_or_self.join(" AND ")
    end
    ...
    def record_to_ferret_document(record)
      ...
      doc[:ar_class] = record.class.to_s
